...
func InitAliClient(appid, appPrivateKey, aesKey, appPublicKeyFilePath, aliPublicKeyFilePath, aliRootKeyFilePath, notifyUrl string, isSaveHandlerLog bool, zfbMiniImp ZfbMiniImp) error {
	appPublicKey, err := gb.ReadFileContent(appPublicKeyFilePath)
	if err != nil {
		return err
	}
	aliPublicKey, err := gb.ReadFileContent(aliPublicKeyFilePath)
	if err != nil {
		return err
	}
	aliRootKey, err := gb.ReadFileContent(aliRootKeyFilePath)
	if err != nil {
		return err
	}
...
func (a *ZFBClient) RegisterHandlers(r *gin.RouterGroup) {
	r.Use(gb.GinLogSetModuleName("支付宝"))
	r.POST("/zfb/login", gb.GinLogSetOptionName("小程序登录", a.IsSaveHandlerLog), a.login)
	r.POST("/zfb/notify", gb.GinLogSetOptionName("支付异步回调", a.IsSaveHandlerLog), a.notify)
	r.POST("/zfb/pay", gb.GinLogSetOptionName("支付请求", a.IsSaveHandlerLog), a.pay)
	r.POST("/zfb/refund", gb.GinLogSetOptionName("退款请求", a.IsSaveHandlerLog), a.refund)
}
...
func (a *ZFBClient) login(c *gin.Context) {
	...
	if err != nil || session.OpenId == "" {
		gb.ResponseError(c, gb.ErrRequestAli.WithMessage("获取支付宝小程序用户会话代码失败"))
		return
	}
...
func (a *ZFBClient) pay(c *gin.Context) {
	...
	if err != nil {
		gb.ResponseError(c, gb.ErrRequestAliPay.WithMessage(err.Error()))
		return
	}
	...
	if aliRsp.StatusCode != 10000 {
		gb.ResponseError(c, gb.ErrRequestAliPay.WithMessage(aliRsp.ErrResponse.Message))
		return
	}
	gb.ResponseSuccess(c, gin.H{
...
func (a *ZFBClient) refund(c *gin.Context) {
	...
	if refund.StatusCode != 10000 {
		gb.ResponseError(c, gb.ErrRequestAliPay.WithMessage(refund.ErrResponse.Message))
		return
	}
	gb.ResponseSuccess(c, gin.H{
